...
- **`balanced`** (`--strategy balanced`) keeps every account's weekly windows on their own schedule, so the fleet's total quota is actually consumed instead of one account running dry days early while another expires unused. Each account is scored by how far ahead of pace its weekly windows are — `used% − expected%`, where `expected%` is the fraction of the 7-day window already elapsed, taken over the 7d window and every `--model` window (the one further ahead binds, since model work debits both) — and below the threshold the engine moves to the account **furthest behind** its schedule once the active one is ahead of it by `hysteresisPct` (15 is a sensible value here: between resets a stay lasts until the active account has burned at least 15 points of its weekly window more than the next account has — 30 with only two accounts — so at the 2–4 points an hour a busy fleet burns, balancing moves are hours apart; the cooldown is the only hard floor). At or above the threshold it leaves for the lowest-scoring healthy account without waiting for the margin. A `preferredAccount` (alias, slot number, or email) has `preferencePct` (default 10) subtracted from its score: it wins near-ties and keeps the work until it is further ahead of its schedule than the alternatives by more than the bias — a bounded preference for, say, billing the company account, not a spend order. Each poll logs the per-account pace deviation so the balance can be read off the log.
