...
- **`balanced`** (`--strategy balanced`) keeps every account's weekly windows on their own schedule, so the fleet's total quota is actually consumed instead of one account running dry days early while another expires unused. Each account is scored by how far ahead of pace its weekly windows are — `used% − expected%`, where `expected%` is the fraction of the 7-day window already elapsed, taken over the 7d window and every `--model` window (the one further ahead binds, since model work debits both) — and below the threshold the engine moves to the account **furthest behind** its schedule once the active one is ahead of it by `hysteresisPct` (15 is a sensible value here: between resets a stay lasts until the active account has burned at least 15 points of its weekly window more than the next account has — 30 with only two accounts — so at the 2–4 points an hour a busy fleet burns, balancing moves are hours apart; the cooldown is the only hard floor. That bound assumes every window reports its reset time — a window without one scores as if its week had just begun, and the passage of time alone can then move the scores). At or above the threshold it leaves for the lowest-scoring healthy account without waiting for the margin. A `preferredAccount` (alias, slot number, or email) has `preferencePct` (default 10) subtracted from its score: it wins near-ties and keeps the work until it is further ahead of its schedule than the alternatives by more than the bias — a bounded preference for, say, billing the company account, not a spend order. Each poll logs the per-account pace deviation so the balance can be read off the log. When the `--model` window(s) leave no account to land on but the 5h/7d windows still have room, the tick is decided on 5h/7d alone (`modelFallback` in the poll) so the weekly quota keeps being balanced; the normal view returns as soon as an account with model room can be landed on. Under `balanced` the anti-flip bar that keeps the engine from undoing its last move applies only to an account that cannot be landed on in the current view; the account it just left is returned to as soon as it can be landed on again, and the landing gate plus the pace hysteresis are what keep two accounts from ping-ponging.
