...
cswap auto --home work                # always come back to this account once it resets
...
- **A home account** (`--home`, or `cswap config set autoswitch.home`): name the account whose quota you want **spent first** — by alias, slot number, or email. Everything else becomes overflow: you run on home until it hits the threshold, fall back while it is spent, and are moved back the moment its window resets. Note the direction — naming an account makes it burn sooner, not later; there is no setting for "save this one for last". The engine still leaves it when it hits the threshold, but it now **comes back on its own** as soon as that account's window resets, instead of stranding you on the stand-in until something else happens to move you. While you are on a healthy home, `consume-first` is held back: naming an account is an explicit standing instruction, and letting both rules run turns into an endless home ↔ peer shuffle. Departures at or above the threshold are unaffected. A home that is disabled, quarantined, metered (API-key, unless `--include-api-key-accounts`), or whose usage cannot be read is simply not returned to.
...
cswap config set autoswitch.home work      # prefer this account; return to it once it resets
